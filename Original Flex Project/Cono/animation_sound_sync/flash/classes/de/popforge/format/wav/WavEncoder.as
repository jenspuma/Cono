package de.popforge.format.wav
{
	...
	{
		static public function encodeVector( samples: Vector.<Vector.<Number>>, channels: uint, bits: uint, rate: uint ): ByteArray
		{
		...
		static private function createData( samples: Vector.<Vector.<Number>>, channels: uint, bits: uint, rate: uint ): ByteArray
		{
			var bytes: ByteArray = new ByteArray();
			bytes.endian = Endian.LITTLE_ENDIAN;
			...
			var numSamples: int = samples[0].length;
